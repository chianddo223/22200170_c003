...
// 이 곳에 생각 정리와 코드 구현
// 1~5사이에 랜덤으로 수를 고르고 인원이 2일 때 다시 고르기. 2보다 작다면 인원 +1해주기.
// 고른 수 리턴해주기 
int random_num;

while (1){
    random_num = (rand() % 5); 
    if(persons[random_num] == 2){
        continue;
    }else{
        persons[random_num] += 1;
        return random_num + 1;
    }
}
}
...
// 이 곳에 생각 정리와 코드 구현
// 이름과 방 호수가 배열에 순서대로 저장되어있으니 남학생 명단과 여학생 명단은 그 수만큼 반복으로 순서대로 출력한다.
// 방 명단을 전부 돌면서 현재 찾고 있는 방 호수와 같을 때 그 index의 이름 출력해주기. (방 호수 한개당 명단 6개를 다 돈다.)
